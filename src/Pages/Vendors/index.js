...
export default function Vendors() {
  ...
  async function searchDataApi() {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}${API_URLS.create_get_vendor_api}?page=${page}&count=${pageCount}&search=${searchValue}`,
        {
          headers: {
  ...
  async function getvendorFn() {
    setloding(true);
    try {
      const response = await axios.get(
        `${baseUrl}${API_URLS.create_get_vendor_api}?page=${page}&count=${pageCount}`,
        {
          headers: {
            Authorization: localStorage.getItem("erp_token"),
            "Content-Type": "application/json",
  ...
  function globalCheckBox() {
    if (checkedBox.length === vendor.length) {
      setCheckedBox([]);
    } else {
      setCheckedBox(
    ...
    if (window.confirm("Are You Confirm ?") === true) {
      ...
      try {
        const response = await axios.delete(
          `${baseUrl}${API_URLS.delete_vendor_by_id}?ids=${checkedBox}`,
          {
            headers: {
              Authorization: localStorage.getItem("erp_token"),
              "Content-Type": "application/json",
                ...
                >
                  <span onClick={() => globalCheckBox()}>
                    <Checkbox
                      checked={
                        checkedBox.length !== 0 &&
                        checkedBox.length === vendor.length
                          ? true
                          : false
                      }
                    />

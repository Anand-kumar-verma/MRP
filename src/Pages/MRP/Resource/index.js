...
import { deleteRources } from "../../../Services/Resources/DeleteResources";
...
export default function Resource() {
  ...
  function globalCheckBox() {
    if (checkedBox.length === resource.length) {
      setCheckedBox([]);
    } else {
      setCheckedBox(
        resource.map((singleItem, index) => {
          return Number(singleItem.resource_id);
        })
      );
    }
  }
  ...
  const columns = [
    ...
    { id: "code", label: "Resource Name", minWidth: 170, align: "" },
    {
      id: "population",
      label: "Description",
      minWidth: 170,
    ...
    <>
      <div className="w-full flex justify-between gap-2 items-center py-2  px-4  text-white">
        ...
        <div className="flex gap-1">
          ...
          <p
            onClick={() => navigate("/mrp/resource/create-resource")}
                ...
                >
                  <span onClick={() => globalCheckBox()}>
                    <Checkbox
                      checked={
                        checkedBox.length !== 0 &&
                        checkedBox.length === resource.length
                          ? true
                          : false
                      }
                    />
                  ...
                  <StyledTableRow key={index}>
                    <StyledTableCell component="th" scope="row">
                      <div className=" flex justify-center">
                        <span
                          onClick={() =>
                            singleCheckBox(Number(row?.resource_id))
                          }
                        >
                          <Checkbox
                            checked={
                              checkedBox.includes(Number(row?.resource_id))
                                ? true
                                : false
                            }
                          />

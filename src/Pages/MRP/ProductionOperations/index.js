...
import { deleteOperationsItesFn } from "../../../Services/ProductionOperations/DeleteOperationsItems";
...
export default function ProductionOperations() {
  ...
  const [produntionOperationList, setproduntionOperationList] = React.useState(
    []
  );
  ...
  React.useEffect(() => {
    getProductionOperationsList({
      ...
      page,
      pageCount,
      setproduntionPhaseList: setproduntionOperationList,
      setSubData,
      searchValue,
    });
  }, [dummy, page, pageCount, searchValue]);

  console.log(produntionOperationList);

  function globalCheckBox() {
    if (checkedBox.length === produntionOperationList.length) {
      setCheckedBox([]);
    } else {
      setCheckedBox(
        produntionOperationList.map((singleItem, index) => {
  ...
  const columns = [
    ...
    // { id: 'code', label: 'Cost/hours', minWidth: 170 ,align: '',},
    {
      id: "population",
      label: "Stage",
      minWidth: 170,
    ...
    <>
      <div className="w-full flex justify-between gap-2 items-center py-2  px-4  text-white">
        ...
        <div className="flex gap-1">
          ...
          <p
            onClick={() =>
              navigate("/mrp/production-operation/create-production-operation")
            }
      ...
      <Paper sx={{ width: "100%", overflow: "hidden", height: "100%" }}>
        <TableContainer sx={{ maxheight: "100%" }}>
          <Table stickyHeader aria-label="sticky table">
                ...
                >
                  <span onClick={() => globalCheckBox()}>
                    <Checkbox
                      checked={
                        checkedBox.length !== 0 &&
                        checkedBox.length === produntionOperationList.length
                          ? true
                          : false
                      }
                    />
            ...
            <TableBody>
              {produntionOperationList.length === 0 ? (
                <div className="flex w-full justify-center">
                  <p>No Data Found</p>
                </div>
              ) : (
                produntionOperationList.map((row, index) => (
                  <StyledTableRow key={index}>
                    ...
                    {/* <StyledTableCell component="th" scope="row" className='capitalize'>{row?.cost_per_hour}</StyledTableCell> */}
                    <StyledTableCell component="th" scope="row">
                      {row?.stage}
                    </StyledTableCell>
                    {/* <StyledTableCell component="th" scope="row" >{row?.time}</StyledTableCell> */}
                    {/* <StyledTableCell component="th" scope="row" >{row?.total_cost}</StyledTableCell> */}

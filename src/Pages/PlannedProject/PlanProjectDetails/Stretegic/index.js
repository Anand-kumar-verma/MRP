...
const StregicDetailsData = ({ formik }) => {
  ...
  useEffect(() => {
    ...
    //set phase
    const newrows = data?.phases_on_planning_project?.map((i, index) => {
      return {
        id: index + 1,
        ...
        end_date: i?.start_date?.slice(0, 10) || "",
        idd:i?.id
      };
    });
    ...
    //set milestone
    const newMileStones = data?.phases_on_planning_project?.flatMap(
      (i, firstIndex) => {
        return i?.milestone_on_phases_on_planning_project?.map(
          (item, index) => {
            return {
    ...
    const newmaterial = data?.phases_on_planning_project?.flatMap(
      (i, firstIndex) => {
        return i?.milestone_on_phases_on_planning_project?.flatMap(
          (item, index) => {
            return item?.materials_on_planning_project?.map((material) => {
              return {
                id: firstIndex + 1, // This will give you 0, 1, 2, ...
                material: material?.materials?.name || "",
                bom_id: material?.materials?.bom_id || "",
                phase: item?.milestone_name || "",
                cost: material?.materials?.varients?.purchasing_price || "",
                time: "0",
                varients: material?.materials?.varients || [],
                v_id: material?.materials?.varients?.id || "",
                qnt: material?.quantity || 1,
                idd:material?.id
              };
            });
          }
    ...
    const newOperationData = data?.phases_on_planning_project?.flatMap(
      (i, firstIndex) => {
        return i?.milestone_on_phases_on_planning_project?.flatMap(
          (item, index) => {
            return item?.operations_on_planning_project?.map((operation) => {
              return {
                id: firstIndex + 1, // This will give you 0, 1, 2, ...
                ...
                start_date: operation?.start_date?.slice(0, 10) || "",
                end_date: operation?.start_date?.slice(0, 10) || "",
                idd:operation?.id
              };
            });
          }
  ...
  console.log(formik.values.phase_data);
...
export default StregicDetailsData;

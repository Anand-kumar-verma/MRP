...
const Operational = ({ formik }) => {
  ...
  useEffect(() => {
    if (projectData) {
      //set phase
      const newrows = projectData?.work_order_or_task_on_estimate_project?.map(
        (i, index) => {
          return {
            id: index + 1,
      ...
      const newmaterial =
        projectData?.work_order_or_task_on_estimate_project?.flatMap(
          (i, firstIndex) => {
            return i?.materials_on_estimate_project?.map((item, index) => {
              return {
      ...
      const newOperationData =
        projectData?.work_order_or_task_on_estimate_project?.flatMap(
          (i, firstIndex) => {
            return i?.operations_on_estimate_project?.map((item, index) => {
              return {
                id: firstIndex + 1, // This will give you 0, 1, 2, ...

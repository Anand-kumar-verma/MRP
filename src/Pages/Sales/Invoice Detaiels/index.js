...
export default function InvoiceDatials() {
  const { id } = useParams();
  const [data, setData] = useState([]);
  const Invoice = () => {
    axiosInstance
      .get(`invoice-list/?invoice_id=${id}`)
      .then((response) => {
          ...
          <div id="pdf" className=" w-full h-full px-10 py-5">
            <div className="lg:flex flex-row justify-between items-center  border-b-2">
              ...
              <div className="flex flex-col my-5">
                ...
                <div className="flex">
                  <p className="font-bold">City :</p>
                  <p className="px-2">{data?.[0]?.customer?.address?.city}</p>
                </div>
                <div className="flex">
                  <p className="font-bold">State :</p>
                  <p className="px-2">{data?.[0]?.customer?.address?.state}</p>
                </div>
                <div className="flex">
                  <p className="font-bold">Country :</p>
                  <p className="px-2">
                    {data?.[0]?.customer?.address?.country}
                  </p>
                </div>
                <div className="flex">
                  <p className="font-bold">GSTIN :</p>
                  <p className="px-2">{data?.[0]?.customer?.gst_number}</p>
                </div>
                <div className="flex">
              ...
              <div className="my-5">
                <TableContainer className="!shadow-none" component={Paper}>
                  <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    ...
                    <TableBody>
                      {data?.[0]?.sales_order?.orderItem?.map((item) => {
                        return (
                          <TableRow
                            sx={{
